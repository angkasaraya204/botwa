...
let handler = async (m, { text, usedPrefix, command, conn }) => {
    ...
    try {
        const ressdnime = await fetch(
            "https://api-inference.huggingface.co/models/furusu/SSD-1B-anime",
            {
                headers: { 
        ...
        await conn.sendFile(m.chat, ressdnime, 'ssdanime.jpg', `Result: ${text}`, m);
    } catch (error) {
        throw 'Model sedang loading, silakan coba lagi dalam beberapa menit.'
    }
};

handler.command = handler.help = ['ssdanime'];
handler.tags = ['ai'];
