...
let handler = async (m, { text, usedPrefix, command, conn }) => {
    ...
    try {
        const respastel = await fetch(
            "https://api-inference.huggingface.co/models/stablediffusionapi/pastel-mix-stylized-anime",
            {
                headers: { 
        ...
        await conn.sendFile(m.chat, respastel, 'pastelmix.jpg', `Result: ${text}`, m);
    } catch (error) {
        throw 'Model sedang loading, silakan coba lagi dalam beberapa menit.'
    }
};

handler.command = handler.help = ['pastelmix'];
handler.tags = ['ai'];
