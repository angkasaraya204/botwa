...
let handler = async (m, {
	...
	if (/image/g.test(mime) && !/webp/g.test(mime)) {
        ...
        try {
            ...
            let resmnga = await fetch(`https://api.lolhuman.xyz/api/wmit?apikey=${lol}&img=${outmnga}`)
            let convertmnga = await resmnga.json()
            let imnga = convertmnga.result[0]

            let urlmnga = imnga.map(res => res.urls)
            let teksmnga = `*Judul* : ${imnga.title}\n*Link* : ${urlmnga}\n*Part* : ${imnga.part}\n*Mirip* : ${imnga.similarity}`;
            await conn.sendMessage(m.chat, teksmnga, { quoted: m })
        } catch (e) {
        ...
        await conn.sendMessage(m.chat, { react: { text: `⏱️`, key: m.key } });
        try {
            let res2 = await fetch(`https://api.lolhuman.xyz/api/wmit?apikey=${lol}&img=${encodeURIComponent(text)}`)
            let convert2 = await res2.json()
            let imnga2 = convert2.result[0]

            let urlmnga2 = imnga2.map(res => res.urls)
            let teksmnga2 = `*Judul* : ${imnga2.title}\n*Link* : ${urlmnga2}\n*Part* : ${imnga2.part}\n*Mirip* : ${imnga2.similarity}`;
            await conn.sendMessage(m.chat, teksmnga2, { quoted: m })
        } catch (e) {
