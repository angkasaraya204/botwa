...
let handler = async (m, { text, usedPrefix, command, conn }) => {
    ...
    try {
        const resorange = await fetch(
            "https://api-inference.huggingface.co/models/stablediffusionapi/abyssorangemix2-hardcore",
            {
                headers: { 
        ...
        await conn.sendFile(m.chat, resorange, 'orangemix.jpg', `Result: ${text}`, m);
    } catch (error) {
        throw 'Model sedang loading, silakan coba lagi dalam beberapa menit.'
    }
};

handler.command = handler.help = ['orangemix'];
handler.tags = ['ai'];
