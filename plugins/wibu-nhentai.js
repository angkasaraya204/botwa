    ...
    if (command == "nhensearch") {
        ...
        try {
            let nhen = await fetch(
                `https://api.lolhuman.xyz/api/nhentaisearch?apikey=${lol}&query=${encodeURIComponent(
                    text,
                )}`,
            );
    ...
    if (command == "nhendetail") {
        ...
        try {
            let nhendet = await fetch(
                `https://api.lolhuman.xyz/api/nhentai/${encodeURIComponent(
                    text,
                )}?apikey=${lol}`,
            );
            ...
            await conn.sendMessage(
                m.chat,
                { image: { url: inhendet.image[0] }, caption: tekspages },
                { quoted: m },
            );
        } catch (e) {
            throw `Data Tidak Ditemukan!`;
    ...
    if (command == "nhendl") {
        ...
        try {
            ...
            const stream = doc.pipe(
                fs.createWriteStream(`./media/download/output.pdf`),
            );
            ...
            for (let url of nhendlconv.image) {
                ...
                doc.image(`./media/download/image${i}.jpg`, 0, 0, {
            ...
            await conn.sendFile(
                m.chat,
                `./media/download/output.pdf`,
                `${nhendlconv.title_native}.pdf`,
                `${nhendlconv.title_native}`,
                m,
            );
        } catch (error) {
            throw `Data Tidak Ditemukan!`;
